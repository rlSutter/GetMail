    ...
    public class GetMail : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                if (EmpId == "") { EmpId = "YOUR_EMP_ID"; }
                EmpLogin = WebConfigurationManager.AppSettings["GetChat_EmpLogin"];
                if (EmpLogin == "") { EmpLogin = "YOUR_EMP_LOGIN"; }
            }
            catch { }
            ...
            try
            {
                string Jsonfile = "C:\\Logs\\GetMail-JSON.log"; // Update path as needed
            ...
            {
                logfile = "C:\\Logs\\GetMail.log"; // Update path as needed
                try
                {
            ...
            for (int i = 0; i < ctr; i++)
            {
                ...
                if (EmailAddress[i] != "" && EmailAddress[i] != "root@yourdomain.com" && EmailAddress[i] != "root@yourdomain2.com")
                {
                        ...
                        if (dr.HasRows)
                        {
                            while (dr.Read())
                            {
                                ...
                                if (CON_ID != "")
                                {
                                    ...
                                    {
                                        SqlS = "UPDATE yourdb.dbo.S_CONTACT SET SUPPRESS_EMAIL_FLG='Y' WHERE ROW_ID='" + CON_ID + "'";
                                    }
                                    else {
                                        SqlS = "UPDATE yourdb.dbo.S_CONTACT SET EMAIL_ADDR='' WHERE ROW_ID='" + CON_ID + "'";
                                    }
                                    ...
                                    if (ACTIVITY_ID != "")
                                    {
                                        ...
                                        if (REG_NUM == "" && TRAINER_NUM == "")
                                        {
                                            SqlS = "DELETE FROM yourdb.dbo.CX_CON_DEST WHERE [TYPE]='EMAIL' AND EMAIL_ADDR IS NOT NULL AND LOWER(EMAIL_ADDR)='" + EmailAddress[i].ToLower() + "'";
